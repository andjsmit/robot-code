...
void pre_auton()
{
	...
	// Move all stuff to default
	moveClaw(127); // Move claw
	moveArm(127); //Move arm backwards
...
task autonomous()
{
	...
	leftWheels(127);// Move Forward
	rightWheels(127);
	moveClaw(-127);// Open Claw
	wait1Msec(2100);// wait 2 sec
	leftWheels(0); //Stop moving
	rightWheels(0);
	moveClaw(127);//
	wait1Msec(1000); // Give the claw time to close
	moveArm(-127);
		wait1Msec(500);
	moveArm(idleSpeed);
	//leftWheels(-127);
...
task usercontrol()
{
	...
	while (true)
	{
		...
		if (vexRT[Btn6U]==1){
			...
			moveArm(armUpSpeed); // move move arm up at full speed
		}
		else
		{
			if (vexRT[Btn6D]==1){
				...
				// If nothing pressed
				moveArm(idleSpeed); //This should keep it up
			}
